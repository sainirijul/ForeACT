...
export function MatrixPlot({
  ...
  const xMax = Math.max(
    vThresh * 2,
    ...points.map((p) => p.x_revision_magnitude_pct + 2),
  );
  const yMax = Math.max(
    volThresh * 1.5,
    ...points.map((p) => p.y_volatility_score + 2),
  );

  const getAction = (varClass: string, volClass: string) => {
    const rule = policy?.rules?.find(
      (r) => r.when.includes(varClass) && r.when.includes(volClass),
    );
    ...
    <section className="panel">
      <div className="panel-header">
        <div>
          ...
          <h2>Revision × Volatility Matrix</h2>
        </div>
      </div>
      <Plot
        data={[
        ...
        layout={{
          ...
          xaxis: {
            title: { text: "Absolute latest revision (%)" },
            range: [0, xMax],
            dtick: 1,
          },
          ...
          shapes: [
            {
              ...
              y0: 0,
              y1: volThresh,
              fillcolor: "rgba(220,240,255,0.35)",
              line: { width: 0 },
            },
            {
              type: "rect",
              ...
              y0: 0,
              y1: volThresh,
              fillcolor: "rgba(210,255,220,0.35)",
              line: { width: 0 },
            },
            {
              type: "rect",
              ...
              y0: volThresh,
              y1: yMax,
              fillcolor: "rgba(255,245,210,0.45)",
              line: { width: 0 },
            },
            {
              type: "rect",
              ...
              y0: volThresh,
              y1: yMax,
              fillcolor: "rgba(255,225,210,0.45)",
              line: { width: 0 },
            },
            {
              type: "line",
