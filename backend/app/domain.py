...
CANONICAL_RULES = [
    ...
    RuleSpec(
        ...
        "DecisionCards must trace to DatasetVersion, SemanticFieldModel, AnalysisScope, MethodModel, AlignedForecastModel, and SignalModel.",
        "error",
...
def build_metamodel(field_specs: list[dict[str, Any]], method_spec: dict[str, Any], scope: dict[str, Any], custom_concepts: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    features = [f for f in field_specs if f.get("role") == "feature" and f.get("include_in_model", True)]
    targets = [f for f in field_specs if f.get("role") == "target" and f.get("include_in_model", True)]
    version_fields = [f for f in field_specs if f.get("role") == "forecast_version"]
    horizon_fields = [f for f in field_specs if f.get("role") == "forecast_horizon"]
    scenario_fields = [f for f in field_specs if f.get("role") == "scenario"]

    nodes = [
        {"id": "dataset", "label": "DatasetVersion", "kind": "source", "count": len(field_specs)},
        {"id": "field_model", "label": "SemanticFieldModel", "kind": "metamodel", "count": len(features) + len(targets) + len(version_fields) + len(horizon_fields)},
        {"id": "scope_model", "label": "AnalysisScope", "kind": "metamodel", "count": 1},
        {"id": "scenario_model", "label": "ScenarioAssumptionModel", "kind": "metamodel", "count": len(scenario_fields)},
        {"id": "method_model", "label": "MethodModel", "kind": "metamodel", "count": 2},
        {"id": "aligned_forecast_model", "label": "AlignedForecastModel", "kind": "runtime-model", "count": len(targets)},
        {"id": "signal_model", "label": "SignalModel", "kind": "runtime-model", "count": len(targets) * 2},
        {"id": "decision_model", "label": "DecisionCardModel", "kind": "assurance", "count": len(targets)},
    ]
    edges = [
        {"source": "dataset", "target": "field_model", "label": "T1 profile-to-semantic-model"},
        {"source": "field_model", "target": "scope_model", "label": "binds version/horizon roles"},
        {"source": "scope_model", "target": "aligned_forecast_model", "label": "T2 aligns versions over horizon"},
        {"source": "scenario_model", "target": "aligned_forecast_model", "label": "contextualizes"},
        {"source": "method_model", "target": "signal_model", "label": "T3 computes declared signals"},
        {"source": "aligned_forecast_model", "target": "signal_model", "label": "provides paired forecast values"},
        {"source": "signal_model", "target": "decision_model", "label": "T4 derives actionability"},
    ]
    for i, concept in enumerate(custom_concepts):
        cid = f"custom_{i}_{concept.get('name', 'concept').lower().replace(' ', '_')}"
        nodes.append({"id": cid, "label": concept.get("name", "CustomConcept"), "kind": concept.get("kind", "domain"), "count": 1})
        connects_to = str(concept.get("connects_to") or "ForecastRun")
        target_id = {
            "DatasetVersion": "dataset",
            "SemanticFieldModel": "field_model",
            "AnalysisScope": "scope_model",
            "ScenarioAssumptionModel": "scenario_model",
            "MethodModel": "method_model",
            "AlignedForecastModel": "aligned_forecast_model",
            "SignalModel": "signal_model",
            "DecisionCardModel": "decision_model",
        }.get(connects_to, "aligned_forecast_model")
        edges.append({"source": cid, "target": target_id, "label": "extends"})

    concepts = {
        "DatasetVersion": {"description": "Versioned uploaded dataset with semantic field annotations."},
        "SemanticFieldModel": {"features": features, "targets": targets, "version_fields": version_fields, "horizon_fields": horizon_fields, "scenario_fields": scenario_fields},
        "AnalysisScope": scope,
        "ScenarioAssumptionModel": {
            "description": "Captures planning scenarios and assumptions such as AI data-center commissioning, committed capacity, grid connection probability, and cooling efficiency.",
            "fields": scenario_fields,
        },
        "MethodModel": method_spec,
        "AlignedForecastModel": {"description": "A derived model produced by joining baseline and current forecast versions over the same forecast horizon and target variables."},
        "SignalModel": {"description": "Derived variance, volatility, confidence, and driver-link signals produced from declared methods."},
        "DecisionCardModel": {"description": "Traceable business interpretation generated from signal classes and conformance rules."},
        "CustomConcepts": custom_concepts,
    }
    return {
        "name": "Forecast Actionability DSL",
        "version": "0.4.0",
        "intent": "Make forecast-version comparison, signal computation, actionability, and traceability explicit through model transformations.",
        "concepts": concepts,
        "graph": {"nodes": nodes, "edges": edges},
        "conformance_rules": [asdict(rule) for rule in CANONICAL_RULES],
    }
