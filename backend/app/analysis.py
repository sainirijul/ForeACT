...
def _align_versions(df: pd.DataFrame, target: str, scope: dict[str, Any]) -> tuple[pd.DataFrame, list[str]]:
    ...
    if version_field not in df.columns or horizon_field not in df.columns:
        warnings.append("Forecast version or horizon field is missing; cannot build AlignedForecastModel.")
...
def _conformance(field_specs: list[dict[str, Any]], method_spec: dict[str, Any], scope: dict[str, Any], target_results: list[dict[str, Any]], decision_cards: list[dict[str, Any]], warnings: list[str], custom_concepts: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    trace_ok = bool(decision_cards) and all("AlignedForecastModel" in c.get("trace", {}) for c in decision_cards)
...
def analyze_dataframe(df: pd.DataFrame, field_specs: list[dict[str, Any]] | None = None, methods: dict[str, Any] | None = None, scope: dict[str, Any] | None = None, custom_concepts: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    for target in target_cols:
        ...
        decision_cards.append({
            ...
            "trace": {
                ...
                "SemanticFieldModel": f"{len(feature_cols)} drivers, {len(target_cols)} targets",
                ...
                "MethodModel": f"{method_spec['variance_method']} + {method_spec['volatility_method']}",
                "AlignedForecastModel": f"{len(aligned)} aligned horizon rows",
                "SignalModel": f"variance={latest_variance:.2f}, volatility={volatility_score:.2f}",
            },
        })

    transformations = [
        {"id": "T1", "name": "ProfileToSemanticFieldModel", "input": "DatasetVersion", "output": "SemanticFieldModel", "purpose": "Convert raw CSV columns into typed, editable semantic fields.", "status": "pass"},
        {"id": "T2", "name": "ScopeBindingAndVersionAlignment", "input": "SemanticFieldModel + AnalysisScope", "output": "AlignedForecastModel", "purpose": "Join baseline and current forecast versions for the same forecast horizon and targets.", "status": "pass" if aligned_preview else "warning"},
        {"id": "T3", "name": "MethodModelToSignalModel", "input": "AlignedForecastModel + MethodModel", "output": "VarianceSignal + VolatilitySignal", "purpose": "Apply declared variance and volatility methods rather than hidden calculations.", "status": "pass" if target_results else "warning"},
        {"id": "T4", "name": "SignalModelToDecisionCard", "input": "VarianceSignal + VolatilitySignal + Thresholds", "output": "DecisionCardModel", "purpose": "Transform classified signals into traceable actionability recommendations.", "status": "pass" if decision_cards else "warning"},
    ]
    ...
    return {
        ...
        "aligned_forecast_preview": aligned_preview[:20],
