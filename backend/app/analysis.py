...
def make_demo_energy_data(n_horizons: int = 36, seed: int = 14) -> pd.DataFrame:
    ...
    for c_idx, cycle in enumerate(cycles):
        for h_idx, horizon in enumerate(horizons):
            months_ahead = (horizon.year - cycle.year) * 12 + (
                horizon.month - cycle.month
            )
            t = h_idx
            seasonal = np.sin((h_idx - 2) * 2 * np.pi / 12)
            data_center_committed_mw = 120 + 4.2 * t + 22 * c_idx + rng.normal(0, 5)
            grid_connection_probability_pct = np.clip(
                48
                + 1.0 * t
                + 6.5 * c_idx
                + 9 * np.sin(t * 2 * np.pi / 14)
                + rng.normal(0, 3.5),
                25,
                96,
            )
            power_price_index = (
                ...
                + 0.16 * t
                + 1.5 * c_idx
                + 5 * np.sin(t * 2 * np.pi / 18)
                + rng.normal(0, 1.1)
            )
            avg_temperature_c = 8 + 13 * seasonal + rng.normal(0, 1.0)
            cooling_efficiency_pue = (
                1.32
                - 0.003 * c_idx
                + 0.018 * max(0, avg_temperature_c - 20)
                + rng.normal(0, 0.01)
            )
            renewable_availability_pct = np.clip(
                28
                + 0.4 * t
                + 4.5 * np.sin((t + 2) * 2 * np.pi / 12)
                + rng.normal(0, 2),
                10,
                70,
            )
            ...
            effective_dc_load = (
                data_center_committed_mw
                * 720
                * (grid_connection_probability_pct / 100)
                * cooling_efficiency_pue
            )
            price_effect = -48 * power_price_index
            industrial_effect = 120 * industrial_growth_index
            weather_effect = 210 * max(0, avg_temperature_c - 18) + 120 * max(
                0, 5 - avg_temperature_c
            )
            uncertainty_premium = np.where(h_idx >= 12, 850 * c_idx, 260 * c_idx)

            total_load_mwh_forecast = (
                base_load
                + effective_dc_load
                + price_effect
                + industrial_effect
            ...
            peak_load_mw_forecast = (
                total_load_mwh_forecast / 720 * 1.39
                + 0.38 * data_center_committed_mw
                + 1.5 * max(0, avg_temperature_c - 22)
                + rng.normal(0, 6)
            )
            scenario = (
                "accelerated_ai_load"
                if grid_connection_probability_pct > 72
            ...
            rows.append(
                {
                    ...
                    "scenario": scenario,
                    "feature_1_data_center_committed_mw": round(
                        float(data_center_committed_mw), 2
                    ),
                    "feature_2_grid_connection_probability_pct": round(
                        float(grid_connection_probability_pct), 2
                    ),
                    "feature_3_power_price_index": round(float(power_price_index), 2),
                    "feature_4_avg_temperature_c": round(float(avg_temperature_c), 2),
                    "feature_5_cooling_efficiency_pue": round(
                        float(cooling_efficiency_pue), 3
                    ),
                    "feature_6_industrial_growth_index": round(
                        float(industrial_growth_index), 2
                    ),
                    "feature_7_renewable_availability_pct": round(
                        float(renewable_availability_pct), 2
                    ),
...
def dataset_profile(df: pd.DataFrame) -> dict[str, Any]:
    ...
    role_map = {
        f["role"]: f["name"] for f in default_specs if f.get("include_in_model")
    }
...
def _field_names(field_specs: list[dict[str, Any]], role: str) -> list[str]:
    return [
        ...
        if f.get("role") == role and f.get("include_in_model", True)
    ]
...
def _method_spec(raw: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for threshold in [
        "revision_magnitude_threshold_large",
...
def _recommend(
    revision_magnitude_class: str, volatility_class: str, rules: list[dict[str, Any]]
) -> tuple[str, str]:
...
def _safe_eval_formula(formula: str, env: dict[str, Any]) -> Any:
    ...
    return eval(
        compile(tree, "<custom_formula>", "eval"),
        {"__builtins__": {}, **_ALLOWED_FUNCS},
        env,
    )
...
def _filter_scope(df: pd.DataFrame, scope: dict[str, Any]) -> pd.DataFrame:
    ...
    if horizon in filtered.columns:
        ...
        filtered = filtered[
            (h >= str(scope.get("period_start"))) & (h <= str(scope.get("period_end")))
        ]
...
def _align_versions(
    df: pd.DataFrame, target: str, scope: dict[str, Any]
    ...
    if version_field not in df.columns or horizon_field not in df.columns:
        warnings.append(
            "Forecast version or horizon field is missing; cannot build ForecastComparisonModel."
        )
        return pd.DataFrame(), warnings

    base_v = str(scope.get("baseline_version"))
    curr_v = str(scope.get("current_version"))
    base = df[df[version_field].astype(str) == base_v][[horizon_field, target]].rename(
        columns={target: "baseline"}
    )
    curr = df[df[version_field].astype(str) == curr_v][[horizon_field, target]].rename(
    ...
    if aligned.empty:
        warnings.append(
            f"No aligned rows found for {target} between {base_v} and {curr_v} in the selected horizon."
        )
    return aligned, warnings


def _all_versions_wide(
    df: pd.DataFrame, target: str, scope: dict[str, Any]
) -> pd.DataFrame:
    version_field = scope.get("version_field")
    horizon_field = scope.get("horizon_field")
    if version_field not in df.columns or horizon_field not in df.columns:
        return pd.DataFrame()
    wide = df.pivot_table(
        index=horizon_field, columns=version_field, values=target, aggfunc="mean"
    )
...
def _compute_signals(
...
) -> tuple[pd.Series, pd.Series, str, str, list[str]]:
    warnings: list[str] = []
    ...
    if method_v == "version_to_version_pct":
        ...
        revision_basis = (
            "absolute current-minus-baseline forecast delta over selected horizon"
        )
    elif method_v == "custom":
        env = {
            "baseline": baseline,
            "current": current,
            "all_versions_mean": all_versions.mean(axis=1)
            .reindex(aligned["period"])
            .reset_index(drop=True),
            "all_versions_std": all_versions.std(axis=1)
            .reindex(aligned["period"])
            .reset_index(drop=True),
        }
        revision = pd.Series(
            _safe_eval_formula(
                str(method_spec.get("custom_revision_formula", "")), env
            ),
            index=aligned.index,
        )
        ...
        warnings.append(
            f"Revision method {method_v} does not use forecast versions; fallback used for {target}."
        )
    ...
    if method_vol == "version_dispersion_pct" and not all_versions.empty:
        ...
        volatility_basis = (
            "dispersion across all available forecast versions for each horizon"
        )
    elif method_vol == "revision_instability_pct" and not all_versions.empty:
        revisions = all_versions.pct_change(axis=1) * 100
        volatility = (
            revisions.std(axis=1).reindex(aligned["period"]).reset_index(drop=True)
        )
        volatility = pd.Series(volatility.values, index=aligned.index)
        volatility_basis = (
            "standard deviation of consecutive forecast revisions across versions"
        )
        ...
        env = {
            ...
            .reindex(aligned["period"])
            .reset_index(drop=True),
            "driver_volatility": pd.Series(
                [driver_score] * len(aligned), index=aligned.index
            ),
        }
        volatility = pd.Series(
            _safe_eval_formula(
                str(method_spec.get("custom_volatility_formula", "")), env
            ),
            index=aligned.index,
        )
        ...
        volatility = (
            abs(revision)
            .rolling(window=method_spec["rolling_window"], min_periods=2)
            .std()
            .fillna(0)
        )
        volatility_basis = "fallback rolling standard deviation of revision"
        warnings.append(
            f"Volatility method {method_vol} could not be applied; fallback used for {target}."
        )
    return revision, volatility, revision_basis, volatility_basis, warnings


def _top_driver_links(
        ...
        links.append(
            {
                ...
                "absolute_strength": (
                    None if pd.isna(corr) else round(abs(float(corr)), 3)
                ),
            }
        )
...
def _model_fit_summary(
    ...
    for target in target_cols:
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            X_valid, y_valid, test_size=0.25, random_state=42, shuffle=False
        )
        candidates = {
            "LinearRegression": LinearRegression(),
            "RandomForestRegressor": RandomForestRegressor(
                n_estimators=80, random_state=42, min_samples_leaf=3
            ),
        }
        best = None
...
def _conformance(
    ...
    warnings: list[str],
    custom_concepts: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    ...
    results.append(
        {
            ...
            "severity": "error",
            "message": (
                "Forecast versions are aligned over the selected horizon."
                if aligned_ok
                else "Forecast version alignment is incomplete. "
                + "; ".join(warnings[:2])
            ),
        }
    )
    ...
    results.append(
        {
            ...
            "severity": "error",
            "message": (
                "All selected targets produce revision-magnitude and volatility signals."
                if not missing_targets
                else f"Missing target signals for: {', '.join(missing_targets)}"
            ),
        }
    )
    incomplete = [
        f["name"]
        for f in field_specs
        if f.get("include_in_model")
        and f.get("role")
        in {"target", "feature", "forecast_version", "forecast_horizon"}
        and (
            not f.get("business_name")
            or not f.get("semantic_type")
            or (f.get("role") == "target" and not f.get("unit"))
        )
    ]
    results.append(
        {
            "rule_id": "CR-003",
            "status": "pass" if not incomplete else "warning",
            "severity": "warning",
            "message": (
                "Decision-relevant field metadata is complete."
                if not incomplete
                else f"Review metadata for: {', '.join(incomplete[:8])}"
            ),
        }
    )
    methods_ok = bool(method_spec.get("revision_method")) and bool(
        method_spec.get("volatility_method")
    )
    results.append(
        {
            "rule_id": "CR-004",
            ...
            "severity": "error",
            "message": (
                "Revision and volatility methods are explicitly declared."
                if methods_ok
                else "Declare both revision and volatility methods before analysis."
            ),
        }
    )
    trace_ok = bool(decision_cards) and all(
        "ForecastComparisonModel" in c.get("trace", {}) for c in decision_cards
    )
    results.append(
        {
            "rule_id": "CR-005",
            ...
            "severity": "error",
            "message": (
                "Decision cards trace through the transformation chain."
                if trace_ok
                else "Decision card trace is incomplete."
            ),
        }
    )
    invalid_custom = [
        c
        for c in custom_concepts
        if not c.get("name") or not c.get("description") or not c.get("connects_to")
    ]
    results.append(
        {
            "rule_id": "CR-006",
            "status": "pass" if not invalid_custom else "warning",
            "severity": "warning",
            "message": (
                "Custom metamodel concepts are well-formed."
                if not invalid_custom
                else "Some custom concepts need name, description, and connection."
            ),
        }
    )
    return results
...
def analyze_dataframe(
    ...
    if missing_roles:
        raise ValueError(
            "Semantic mapping is missing required roles: " + ", ".join(missing_roles)
        )
    ...
    if not version_field:
        raise ValueError("A forecast-version field must be selected.")

    if not horizon_field:
        raise ValueError("A forecast-horizon field must be selected.")
    ...
    if not baseline_version:
        raise ValueError("A baseline forecast version must be selected.")

    if not current_version:
        raise ValueError("A current forecast version must be selected.")
    ...
    available_versions = {
        version for version in normalized_version_series.dropna().unique() if version
    }
    ...
    if normalized_baseline == normalized_current:
        raise ValueError("Baseline and current forecast versions must be different.")
    ...
    if (
        ...
        raise ValueError("Forecast period start must not be after period end.")
    ...
    for target in target_cols:
        ...
        revision, volatility, revision_basis, volatility_basis, signal_warnings = (
            _compute_signals(
        ...
        latest_revision = (
            float(aligned["signed_revision_pct"].dropna().iloc[-1])
        ...
        volatility_score = (
            float(aligned["volatility"].mean())
            if aligned["volatility"].notna().any()
            else 0.0
        )
        revision_magnitude_class = _classify_revision_magnitude(
            latest_revision, method_spec
        )
        ...
        series = [
            {
                ...
                "baseline": (
                    ...
                    else round(float(row["baseline"]), 3)
                ),
                "current": (
                    None if pd.isna(row["current"]) else round(float(row["current"]), 3)
                ),
                "signed_revision_pct": (
                    None
                    if pd.isna(row["signed_revision_pct"])
                    else round(float(row["signed_revision_pct"]), 3)
                ),
                "revision_magnitude_pct": (
                    None
                    if pd.isna(row["revision_magnitude_pct"])
                    else round(float(row["revision_magnitude_pct"]), 3)
                ),
                "volatility": (
                    None
                    if pd.isna(row["volatility"])
                    else round(float(row["volatility"]), 3)
                ),
            }
        ...
        matrix_points.append(
            {
                "target": target,
                "x_revision_magnitude_pct": round(abs(latest_revision), 3),
                "y_volatility_score": round(volatility_score, 3),
                "revision_magnitude_class": revision_magnitude_class,
                "volatility_class": volatility_class,
                "recommended_action": action,
            }
    ...
    transformations = [
        {
            "id": "T1",
            "name": "ProfileToFieldModel",
            "input": "DatasetVersion",
            "output": "FieldModel",
            "purpose": "Convert raw CSV columns into typed, editable semantic fields.",
            "status": "pass",
        },
        {
            "id": "T2",
            "name": "ScopeBindingAndVersionAlignment",
            "input": "FieldModel + AnalysisScope",
            "output": "ForecastComparisonModel",
            "purpose": "Join baseline and current forecast versions for the same forecast horizon and targets.",
            "status": "pass" if aligned_preview else "warning",
        },
        {
            "id": "T3",
            "name": "MethodSetToSignalModel",
            "input": "ForecastComparisonModel + MethodSet",
            "output": "RevisionMagnitudeSignal + VolatilitySignal",
            "purpose": "Apply declared revision and volatility methods rather than hidden calculations.",
            "status": "pass" if target_results else "warning",
        },
        {
            "id": "T4",
            "name": "SignalModelToDecisionCard",
            "input": "RevisionMagnitudeSignal + VolatilitySignal + Thresholds",
            "output": "DecisionModel",
            "purpose": "Transform classified signals into traceable actionability recommendations.",
            "status": "pass" if decision_cards else "warning",
        },
    ]
    ...
    return {
        "dataset_profile": {
            **profile,
            ...
            "selected_methods": method_spec,
            "selected_scope": scope_spec,
        },
        "metamodel": build_metamodel(
            field_specs, method_spec, scope_spec, custom_concepts
        ),
