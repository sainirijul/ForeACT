...
REQUIRED_CLASSIFIERS = {
 "ForecastAssuranceProject", "DatasetVersion", "FieldModel", "SemanticField",
 "VersionField", "HorizonField", "TargetField", "MethodSet", "RevisionMethod",
 "VolatilityMethod", "ForecastComparisonModel", "ForecastVintage", "ForecastValue",
 "ForecastRevisionRecord", "ForecastVolatilityRecord", "SignalModel",
 "RevisionMagnitudeSignal", "VolatilitySignal", "ConfidenceSignal",
 "DecisionModel", "DecisionPolicy", "DecisionCard"
}
...
def compile_project_model(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError("Ecore metamodel is missing required classifiers: " + ", ".join(missing))
    ...
    return {
      ...
      "instances": {
        ...
        "TargetField": sum(f.get("role") == "target" and f.get("include_in_model", True) for f in fields),
